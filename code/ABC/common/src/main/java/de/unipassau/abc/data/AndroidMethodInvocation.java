...
public class AndroidMethodInvocation extends MethodInvocation {
  ...
  // Create a more specific android related copy out of a general method invocation
  public AndroidMethodInvocation(MethodInvocation methodInvocation) {
    ...
    isAndroidFragmentCallback =
        owner.isAndroidFragment() && JimpleUtils.isFragmentLifecycle(methodSignature);
    isAndroidActivityCallback =
        owner.isAndroidActivity() && JimpleUtils.isActivityLifecycle(methodSignature);
  }
   ...
   * <p>
   * onCreate(): In the onCreate() method, you perform basic application startup logic that should
   * happen only once for the entire life of the activity. This method receives the parameter
   * savedInstanceState, which is a Bundle object containing the activity's previously saved state.
   * If the activity has never existed before, the value of the Bundle object is null.
   * <p>
   * >> Relevant properties of the bundle are accessed by calling getXXX methods while putXXX
   * methods are used to store the state. Hence, in theory, carving the bundle objects might be
   * enough to "drive" the activity in a specific state.
   *
   * </br>
   * onStart(): The onStart() call makes the activity visible to the user, as the app prepares for
   * the activity to enter the foreground and become interactive. he onStart() method completes very
   * quickly and, as with the Created state, the activity does not stay resident in the Started
   * state. Once this callback finishes, the activity enters the Resumed state, and the system
   * invokes the onResume() method.
   * <p>
   * >> This means we can chain onStart and onResume basically always...
   *
   * </br>
   * onResume(): . This is the state in which the app interacts with the user. The app stays in this
   * state until something happens to take focus away from the app. Such an event might be, for
   * instance, receiving a phone call, the user’s navigating to another activity, or the device
   ...
   * </br>
   * <p>
   * onPause(): When an interruptive event occurs, the activity enters the Paused state, and the
   * system invokes the onPause() callback. user is leaving your activity (though it does not always
   * mean the activity is being destroyed); it indicates that the activity is no longer in the
   ...
   *
   * </br>
   * onStop(): When your activity is no longer visible to the user, it has entered the Stopped
   * state, and the system invokes the onStop() callback. This may occur, for example, when a newly
   * launched activity covers the entire screen. The system may also call onStop() when the activity
   * has finished running, and is about to be terminated.
   *
   *
   * </br>
  ...
  public MethodInvocation clone() {
    // This works as long as super class also class super.clone() within its clone method
