...
public class MockingGenerator {
	 ...
	 * provide inputs to java.util.Scanner. It's hard=coded, but it should make it.
	 * Then for the test it carve out the inputs. Note we assume the last call in
	 * the each test method is the invocation of the method under test
	 * 
	 * @param testClass
	 * @param testMethod
	 * @param carvedTest
	...
	public static void addSystemIn(SootClass testClass,
		...
		if (testClass.declaresFieldByName(SYSTEM_IN_RULE_NAME)) {
			...
			systemInJunit4RuleField = new SootField(SYSTEM_IN_RULE_NAME,
					RefType.v("org.junit.contrib.java.lang.system.TextFromStandardInputStream"), Modifier.PUBLIC);
			//
			// Add annotation
...
//			System.out.println( "TAGS: " + systemInJunit4RuleField.getTags() ); 
			...
			Unit newAssignStmt = Jimple.v().newAssignStmt(local, Jimple.v().newStaticInvokeExpr(Scene.v().getMethod(
	...
	public static void addSystemExit(SootClass testClass,
		...
		if (testClass.declaresFieldByName(SYSTEM_EXIT_RULE_NAME)) {
			...
			systemExitJunit4RuleField = new SootField(SYSTEM_EXIT_RULE_NAME,
					RefType.v("org.junit.contrib.java.lang.system.ExpectedSystemExit"), Modifier.PUBLIC);
			// Add annotation
	...
	private static Value collectExpectedSystemExitValue(
		...
		for (MethodInvocation methodInvocation : subsequentCalls) {
			if (systemExitMethodMatcher.matches(methodInvocation)) {
				logger.debug("MockingGenerator.collectExpectedSystemExitValue() System.exit call subsumed by "
						+ methodInvocationToBeCarved);
