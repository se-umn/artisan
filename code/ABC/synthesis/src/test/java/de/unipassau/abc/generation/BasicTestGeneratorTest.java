...
public class BasicTestGeneratorTest {
	...
	public void testEndToEnd() throws FileNotFoundException, IOException, ABCException {
		...
		for (MethodInvocation mi : parsedTrace.getUIThreadParsedTrace().getFirst().getOrderedMethodInvocations()) {
			if (targetSignature.equals(mi.getMethodSignature())) {
				targetMethodsInvocations.add(mi);
			}
		}
