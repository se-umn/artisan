 ...
 */
public class PatchedClassAnalyzer extends ClassProbesVisitor implements IFilterContext {
    ...
    private IFilter[] additionalFilters;
     ...
     *                   instances
     */
    public PatchedClassAnalyzer(final ClassCoverageImpl coverage, final boolean[] probes, final StringPool stringPool,
    ...
    public MethodProbesVisitor visitMethod(final int access, final String name, final String desc,
        ...
        return new MethodAnalyzer(builder) {
            ...
            public void accept(final MethodNode methodNode, final MethodVisitor methodVisitor) {
                ...
                addMethodCoverage(stringPool.get(name), stringPool.get(desc), stringPool.get(signature), builder,
                        methodNode);
            }
        };
    }

    private void addMethodCoverage(final String name, final String desc, final String signature,
            final InstructionsBuilder icc, final MethodNode methodNode) {
        final MethodCoverageCalculator mcc = new MethodCoverageCalculator(icc.getInstructions());
        // Invoke the original filters
        filter.filter(methodNode, this, mcc);
        // Invoke the additional filters
        for (IFilter additionalFilter : additionalFilters) {
            additionalFilter.filter(methodNode, this, mcc);
        }
