...
/**
 * TODO Array instantiation and access must be transformed into
 * 
...
public class SceneInstrumenterWithMethodParameters implements Extension {
    ...
    private static final String MONITOR_CLASS = "dynCG2.Monitor";
    ...
    public void run() {
        ...
        // System.out.println("instrument 3rd party libraries as well: " +
        // g_instr3rdparty);
        try {
            instrument();
        ...
        //
        // if (opts.dumpJimple()) {
        // String fnJimple = soot.options.Options.v().output_dir() +
        // File.separator + utils.getAPKName()
        // + "_JimpleInstrumented.out";
        // // String fnJimple = Util.getCreateBaseOutPath() +
        // // "JimpleInstrumented.out";
    ...
    /**
     * Instrumentation: - inject onMethodEntry at the beginning of application
     * methods - inject onMethodEntry at the before each non-application (lib,
     * android) method - inject onReturnInfo after each invoked method
     * 
     * @throws XmlPullParserException
     * @throws IOException
     */

    // private Set<SootClass> getChildrenOfIncluding(Collection<SootClass>
    // classes) {
    // return Stream
    // .concat(classes.stream().filter(c ->
    ...
    public void instrument() throws IOException, XmlPullParserException {
        ...
        /*
         * Iterate over all the user classes. Those include any class which is
         * generated to fit android (i.e., 1 class 1 smali)
         */
        ...
        while (applicationClassesIterator.hasNext()) {
            ...
            /*
             * Traverse all methods of this class at include the monitor
             * initialize and the onMethodEnter for application methods
             */
            ...
            while (methodsIterator.hasNext()) {
                ...
                /*
                 * Forces soot to (re)load the entire class body using
                 * retrieveActiveBody()
                 */
                ...
                /*
                 * To ensure monitor is initialized we inject initialization
                 * code. Note that we apply this later because we need to put
                 * this code at the very beginning of the method (possibly
                 * BEFORE the methodBegins probe!). THIS IS NEED IF THERE"S
                 * CLINIT IN THE ENTRY CLASSES< BUT WE DO NOT HAVE ENTRY CLASSES
                 * SOMEHOW ?
                 */
                /*
                 * To Ensure we log app methods being called we insert an
                 * methodEnter probe at the beginning of the
                 * currentlyInstrumentedMethod. Private methods are tagged as
                 * such
                 */
                ...
                /*
                 * Inject instrumentation code before any return/throw statement
                 * in the method. This might also require to explicitly
                 * introdyce try/catch statements to capture (and rethrow) also
                ...
                /*
                 * Instrument the methods which do not belong to user called but
                 * are called by the currentlyInstrumentedMethod
                 */
                // int nCurCallsite = -> TODO Not sure how to use this and why
                // we care...
                /*
                 * TODO CFG is useful because it carries a lot of info, but it
                 * also skip some, e.g., it does not consider call sites inside
                 * method of abstract classes which can be actually invoked. So
                 * we resort going through the body of the method and doing
                 * stuff manually, Probably we can get rid of this entire
                 * DuafDroid thingy ...
                 * 
                 */
                ...
                /*
                 * TODO: To Ensure we log app when the app is terminated using
                 * System.exit we insert a probe before calling System.exit
                 */
     ...
     * events, but we limit ourselves to the one we can trigger during testing
     * using ActivityController by Robolectrics
     * 
    ...
    private void makeInterestingLifeCycleEventsExplicit(SootClass currentlyInstrumentedSootClass) {
        ...
        if (Scene.v().getActiveHierarchy().isClassSubclassOf(currentlyInstrumentedSootClass, androidActivity)) {
            /*
             * For each method in androidActivity which starts with on check if
             * the currentlyInstrumentedSootClass override it or not. If not
             * create a synthetic method which simply calls super...
             */
            for (SootMethod androidActivityMethod : androidActivity.getMethods()) {
                ...
                if (interestingAndroidLifecycleCallbacks.contains(androidActivityMethod.getName())
                    ...
                    if (!currentlyInstrumentedSootClass.declaresMethod(androidActivityMethod.getSubSignature())) {
                        ...
                        /*
                         * Lookup the hierarchy until we find the one
                         * implementing this method and use that as super for
                         * the call... Worst case is androidActivity class
                         */
                        ...
                        /*
                         * Implement the override method by calling the
                         * corresponding super method via a specialinvoke and
                         * handling its return type
                         */
                        ...
                        Local thisLocal = UtilInstrumenter2.generateFreshLocal(body,
                        ...
                        for (int number = 0; number < method.getParameterTypes().size(); number++) {
                            ...
                            Local parameterLocal = UtilInstrumenter2.generateFreshLocal(body, type);
            ...
            /*
             * For each interesting life cycle method in androidFragment check
             * if the currentlyInstrumentedSootClass override it or not. If not
             * create a synthetic method which simply calls super...
             */
        ...
        for (final Iterator<Unit> iter = currentlyInstrumentedMethodBodyUnitChain.snapshotIterator(); iter.hasNext();) {
            ...
            currentUnit.apply(new AbstractStmtSwitch() {
                ...
                private void wrapLibraryInvocationIfAny(Stmt csStmt) {
                    ...
                    /*
                     * Object[] methodParameters - We need to wrap this into an
                     * array of objects
                     */
                    ...
                    Pair<Value, List<Unit>> tmpArgsListAndInstructions = UtilInstrumenter2.generateParameterArray(
                    ...
                    /*
                     * Insert the instructions to create the array before using
                     * it
                     */
                    ...
                    /*
                     * We defer the instrumentation to enable holding the value
                     * of original owner back
                     */

                    /*
                     * Preparing the call to monitorOnReturnInto ->
                     * dynCG2.Monitor.returnInto(Object csOwnerOrEmptyString,
                     * String methodSignature, Object returnValueOrNullIfVoid)
                     */
                    ...
                    if (csStmt instanceof AssignStmt) {
                        /*
                         * [1] If the assign re-assign the owner, we need to log
                         * the "original" owner not the returned one... so we
                         * need to assign the original owner to a new local
                         * before the assign stmt overwrites it. In case of
                         * aliasing this might fail since it requires casting...
                         * so we need to check for that as well ?(TODO)
                         */
                        if (((AssignStmt) csStmt).getLeftOp().equals(csOwner)
                                && !JimpleUtils.isString(csOwner.getType())) {

                            Local originalOwnerHolder = UtilInstrumenter2
                    ...
                    /*
                     * If csStmt is InvokeStmt its return value is not used, so
                     * soot does not link that to any local and we cannot read
                     * the return value directly. We replace the invoke stmt
                     * with an assignment stmt to a local, and use that assigned
                     * local to capture the return value (unless void)
                     * 
                     * Unfortunately, this changes the CFG since the original
                     * invoke is removed (otherwise it will be executed twice!).
                     * but we might not even use the CFG anymore...
                     */

                    if (csStmt instanceof InvokeStmt && !(invokedMethod.getReturnType() instanceof VoidType)) {
                        /*
                         * We need to replace the original InvokeStmt with a
                         * AssignStmt and then return the Local assigned to
                         */
                        returnValueFromCallSite = UtilInstrumenter2.generateFreshLocal(currentlyInstrumentedMethodBody,
                    ...
                    monitorOnReturnIntoParameters.add(UtilInstrumenter2.generateCorrectObject(
                    ...
                    /*
                     * Finally inject the instrumentation code AFTER the target
                     * which is either the original location or the new one
                     * stmt, after replacin invoke with assign
                     */
                ...
                /**
                 * Arrays do not have an INIT function, they are not objects, so
                 * we fake one the moment we see newarray. We need to wrap the
                ...
                private void instrumentArrayInitExpression(AssignStmt stmt) {
                    ...
                    if (opts.debugOut()) {
                        System.out.println("\t\t\t WRAP arrayInit " + stmt + " inside "
                                + currentlyInstrumentedMethod.getSignature());
                    }
                ...
                private void instrumentArrayAccess(AssignStmt stmt) {
                    ...
                    if (opts.debugOut()) {
                        System.out.println("\t\t\t WRAP arrayGet " + stmt + " inside "
                                + currentlyInstrumentedMethod.getSignature());
                    }
                    ...
                    // We need to replace only the first (actually only one) otherwise nested arrays cannot be handled correctly [][][]
                    String fakeMethodSignature = "<" + array.getType() + ": " +  array.getType().toString().replaceFirst("\\[\\]", "") + " get(int)>";
                    ...
                    // String context
                    monitorOnReturnIntoParameters.add(StringConstant.v(currentlyInstrumentedMethod.getSignature()));
                    // We need to wrap the return value to deal with primitives before invoking Monitor
                    monitorOnReturnIntoParameters.add(UtilInstrumenter2.generateCorrectObject(
                ...
                /**
                 * Fake a call to a generic method STORE of the array. In JIMPLE
                 * those are ALWAYS ? $r10[0] = $r0 -> void $r10.store( $r0, 0 )
                 * 
                 */
                private void instrumentArrayStore(AssignStmt stmt) {
                    ...
                    if (opts.debugOut()) {
                        System.out.println("\t\t\t WRAP arrayStore " + stmt + " inside "
                                + currentlyInstrumentedMethod.getSignature());
                    }
    ...
    private void instumentMethodBegins(SootMethod currentlyInstrumentedMethod,
            /*
             * Need to generate fresh locals and access other elements like
             * "this.local" - TODO Is this.local the method owner or the method
             * itself?
             */
            Body currentlyInstrumentedMethodBody,
            /*
             * Need to inject the code inside the currentlyInstrumentedMethod -
             * TODO Can't we simply get a new instance of this or shall we pass
             * it around ?
             */
         ...
         * 
         * NOTE parameters must passed using Object[], so primitive types must
         * be suitably Boxed
         */
        ...
        if (currentlyInstrumentedMethod.isStatic()) {
            /*
             * Static methods cannot refer to instances. And null might simply
             * break the code...
             */
            methodStartParameters.add(NullConstant.v());
        } else if (currentlyInstrumentedMethod.isConstructor()) {
            /*
             * Constructors are tricky: they let you refer to this, but this is
             * initialized by a chain of super.init. Instead of logging this
             * mid-way during the constructor execution, we report this when the
             * constructor returns
             */
            methodStartParameters.add(NullConstant.v());
        } else {
        ...
        Pair<Value, List<Unit>> tmpArgsListAndInstructions = UtilInstrumenter2.generateParameterArrayFromParametersLocal(
        ...
        /*
         * Add the code which calls "monitorOnEnter" unless the method is a
         * synthetic method we generated. In that case, invoke a
         * monitorOnEnterSynthetic.
         */
        ...
        /*
         * Since monitorOnEnter requires "THIS" and the input parameters, we
         * cannot invoke it before both this (if any) and the input parameters
         * have been assigned to their Locals. Not that for constructor we
         * cannot log at this position the reference to this, but we need to
         * wait when super.init returns. For this reason we log the reference to
         * this only before returning.
         */
    ...
    private void instrumentMethodEnds(SootMethod currentlyInstrumentedSootMethod,
         ...
         * exceptional branches and finally blocks we build the
         * exceptionalUnitGraph (a CFG that includes the exceptional branches),
         * get the exit points of the method, i.e., the tails and insert the
         * instrumentation right before each of them.
         */
        ...
        /*
         * Tails also includes throw stmt which will be captured by catch
         * blocks/traps inside the code. Those are not really method exit points
         * so we need to rule them out.
         * 
         * For every ThrowInst or ThrowStmt Unit which may explicitly throw an
         * exception that would be caught by a Trap in the Body, there will be
         * an edge from the throw Unit to the Trap handler's first Unit.
         * 
         ...
         * execution before the handler starts execution). If the excepting Unit
         * might have the side effect of changing some field, then there will
         * definitely be an edge from the excepting Unit itself to its handlers,
         * since the side effect might occur before the exception is raised. If
         * the excepting Unit has no side effects, then parameters passed to the
         * ExceptionalUnitGraph constructor determine whether or not there is an
         * edge from the excepting Unit itself to the handler Unit.
         */
        ...
        for (Unit exitPoint : potentialExitPoints) {
            ...
            exitPoint.apply(new AbstractStmtSwitch() {
                ...
                public void caseThrowStmt(ThrowStmt stmt) {
                    ...
                    onReturnIntoFromExceptionArgs.add(UtilInstrumenter2
                    ...
                    if (opts.debugOut()) {
                        System.out.println(
                                "\t\t\t Injected returnInto at the end of the method before throwing exception "
                ...
                public void caseReturnStmt(ReturnStmt stmt) {
                    ...
                    onReturnIntoArgs.add(UtilInstrumenter2.generateCorrectObject(currentlyInstrumentedMethodBody,
                ...
                public void caseReturnVoidStmt(ReturnVoidStmt stmt) {
                    ...
                    if (opts.debugOut()) {
                        System.out
                                .println("\t\t\t Injected returnInto at the end of the method before returning (void)");
                    }
                }
            });
    ...
    /**
     * instrument 'terminate' monitor before 'System.exit' as a special
     * treatment for Java programs
     * 
     * @param cfg
    ...
    private boolean skipMethod(SootMethod currentlyInstrumentedSootMethod) {
        /*
         * Complex Data Structure given by duafdroid. TODO Not sure this is
         * really needed
         */
        // CFG cfg =
    ...
    public void wrapTryCatchAllBlocks(SootMethod currentlyInstrumentedMethod) {
        ...
        Local er1 = UtilInstrumenter2.generateFreshLocal(b, RefType.v(sce));
        Local er2 = UtilInstrumenter2.generateFreshLocal(b, RefType.v(sce));

        // Local er1 = utils.createUniqueLocal(b, "er1", RefType.v(sce));
        // Local er2 = utils.createUniqueLocal(b, "$er2", RefType.v(sce));
        // the ID statement
        /*
         * We do not tag our code since it does not change the semantic, plus we
         * should not skip instrumenting it with returnInto calls
         */
        ...
        if (sLast instanceof IdentityStmt) {
            ...
            /*
             * Stmt tgt = (Stmt)utils.getFirstNonIdUnit(pchain); if (tgt !=
             * null) tgt = (Stmt)pchain.getPredOf(tgt); if (tgt == null) tgt =
             * sLast; InstrumManager.v().insertAtProbeBottom(pchain, tcProbes,
             * sLast);
             */
