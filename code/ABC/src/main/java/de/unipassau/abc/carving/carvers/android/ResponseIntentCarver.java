...
public class ResponseIntentCarver {
    ...
    public Pair<ExecutionFlowGraph, DataDependencyGraph> carve(ObjectInstance activityUnderTest)
        ...
        if( setResult.getActualParameterInstances().size() > 1 ){
            ...
            return new Pair<ExecutionFlowGraph, DataDependencyGraph>(new ExecutionFlowGraph(), new DataDependencyGraph());
        }
    ...
    private MethodInvocation findSetResultMethod(ObjectInstance activityUnderTest)
            throws CarvingException {
        
        // We cannot directly look for this. We need to find the latest onPause and the last setResult before it
        ...
        MethodInvocationMatcher.keepMatchingInPlace( MethodInvocationMatcher.byMethodName("onPause|onResume"), activityMethods);
        // Remove calls to super
        MethodInvocationMatcher.keepMatchingInPlace( MethodInvocationMatcher.byClass(activityUnderTest.getType()), activityMethods);
        Collections.sort( activityMethods );

        if( activityMethods.size() < 2 ){
            // THIS IS NOT AN ERROR BUT CANNOT HANDLE RIGHT NOW
            throw new CarvingException("TODO Somehow there's no onPause for " + activityUnderTest + " while looking fro a setResult method !");
        }
        ...
        MethodInvocationMatcher.keepMatchingInPlace( MethodInvocationMatcher.byMethodName("setResult"), setResultMethods );
        //
        if( setResultMethods.size() > 1 ){
         // THis is actually an error !
            throw new CarvingException("Wrong expectation for " + activityUnderTest + " foudn more than 1 setResult " + setResultMethods );
        }
