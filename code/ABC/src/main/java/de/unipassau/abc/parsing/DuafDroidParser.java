...
//    public Map<String, Triplette<ExecutionFlowGraph, DataDependencyGraph, CallGraph>> parseTrace(File traceFile)
    public ParsedTrace parseTrace(File traceFile)
        ...
        /*
         * Read the ENTIRE file into a set of string, this might be a problem
         * for long traces... TODO Refector with StreamAPI or sequential access
         */
         ...
         * 
         * We assume that a trace ends when it ends ;) that is System.exit might
        ...
        try (BufferedReader b = new BufferedReader(new FileReader(traceFile))) {
            ...
            while ((currentLine = b.readLine()) != null) {
                ...
                try {
                    ...
                    /*
                     * We must call this every time we start a new trace to
                     * ensure it removes spurious/uncomplete onDestroy methods
                     */
                    ...
                    /*
                     * Remove from the trace threading information and filter
                     * trace entry by thread name
                     */
                    ...
                    case Trace.LIB_METHOD_START_TOKEN:
                        ...
                        /*
                         * This is the basic carving object, a method
                        ...
                        if (methodInvocation.isSynthetic()) {
                            /*
                             * A call which lacks the owner and is not an
                             * instance constructor must be static. Note there's
                             * no way from the methodSignature to distinguish
                             * static vs instance calls but we cannot load the
                             * soot method for syntethic method call generated
                             * by ABC.
                             */
                            ...
                            if (sootMethod != null) {
                                ...
                                /*
                                 * This is a method that we created on the fly,
                                 * it does not exist for real...
                                 */
                                ...
                                /*
                                 * At this point sootMethod might be null for
                                 * special classes like "BuildConfig". In this
                                 * case we rely on heuristics to set the details
                                 * of the methodInvocation. Usually those
                                 * classes have no methods except <clinit>
                                 */
                        ...
                        /*
                         * Method Ownership for non static, non constructor
                         * method (including static initializers)
                         */
                    ...
                    case Trace.EXCEPTION_METHOD_END_TOKEN:
                        ...
                        /*
                         * Retrieve the last method invocation registered Check
                         * that this corresponds to the one we just parsed
                         * Method Signatures must match and owner must match
                        ...
                        /*
                         * At this point we found a matching method call we
                         * update the data structures.
                         */
                        if (JimpleUtils.isConstructor(methodSignature)) {
                            /*
                             * Ownership info for constructors are available
                             * only by the end of the invocation... This is not
                             * 100% accurate but we try to live with that...
                             */
                            ObjectInstance owner = new ObjectInstance(methodOwner);
                            owner.setAndroidActivity(isActivity(sootClass));
                            owner.setAndroidFragment(isFragment(sootClass));
...
//        >> parsedTraceFiles = new HashMap<>();
        ...
        /*
         * Serialize the parsed graphs to files so maybe we can avoid re-parsing
         * the trace over and over and we might be able to limit the memory
