...
public class ABCInstrumentArtificialInvocations extends BodyTransformer {
			...
			currentUnit.apply(new AbstractStmtSwitch() {
				...
				public void caseAssignStmt(AssignStmt stmt) {
					...
					if (stmt.getLeftOp() instanceof Local) {
						...
						System.out.println("We cannot handle assignments to object fields yet !");
						// if ( ! ((FieldRef)
						// stmt.getLeftOp()).getField().isPrivate() ){
						// instrumentPublicFieldAssignmentExpression(body,
						// currentUnit, ((FieldRef)
						// stmt.getLeftOp()).getField(), stmt.getRightOp() );
						// }
					}
	...
	// private void instrumentPublicFieldAssignmentExpression(Body body, Unit
	// currentUnit, //
	// SootField field,
	// Value value) {
	// // TODO Auto-generated method stub
	// String invokeType = "PublicFieldOperation";
	//
	// String fakeMethodSignature = "<abc.File: void set("+value.getType()+")>";
	//
	...
	//
	// generatedBefore.addAll(addTraceStart(invokeType, fakeMethodSignature,
	// parameterList, body));
	//
	// // Since "abc.String" is static we do not trace its owner
	// generatedAfter.addAll(addTraceObject(field.m, fakeMethodSignature));
	//
