...
public class InstrumManager {
	...
	/** Indicates whether anything has been inserted in the program (or at least the instrumenter has been invoked). */
	...
	/** Indicates whether anything has been inserted in the program (or at least the instrumenter has been invoked). */
	public boolean isThereAnyInstrumentation() { return isThereAnyInstrumentation; }
	...
	public void insertBeforeNoRedirect(PatchingChain pchain, List instrumCode, Stmt sTarget) {
		isThereAnyInstrumentation = true;
		for (Object oS : instrumCode)
			pchain.insertBeforeNoRedirect((Unit) oS, sTarget);
	}
	 ...
	 */
	public void insertAtProbeTop(PatchingChain pchain, List instrumCode, Stmt sTargetLocation) {
		isThereAnyInstrumentation = true;
		
		assert !(sTargetLocation instanceof IdentityStmt);
		Stmt sStmtProbeBegin = stmtProbes.get(sTargetLocation);
		insertBeforeRedirect(pchain, instrumCode, sStmtProbeBegin == null? sTargetLocation : sStmtProbeBegin);
		stmtProbes.put(sTargetLocation, (Stmt) instrumCode.get(0));
	}
	public void insertAtProbeBottom(PatchingChain pchain, List instrumCode, Stmt sTargetLocation) {
		isThereAnyInstrumentation = true;
		
		assert !(sTargetLocation instanceof IdentityStmt);
		insertBeforeRedirect(pchain, instrumCode, sTargetLocation);
		if (stmtProbes.get(sTargetLocation) == null)
			stmtProbes.put(sTargetLocation, (Stmt) instrumCode.get(0));  // code also becomes top of probe
	}
	...
	public void insertProbeAt(Edge e, List instrumCode) {
		...
		if (sEdgeProbeBegin == null) {  // probe for edge didn't exist; we need to create one
			...
			if (e.getSrc() == cfg.ENTRY || e.getTgt() == cfg.ENTRY)  // edge EN->firststmt or EX->EN (i.e., entry branch)
				...
				boolean found = false;
				/** hcai: in the presence of exceptional units, the source can be an identity 
				 * statement such as $r1:=@caughtexception 
				 */
				//assert !(e.getSrc().getStmt() instanceof IdentityStmt);
				if (dua.Options.ignoreCatchBlocks) {
					assert !(e.getSrc().getStmt() instanceof IdentityStmt);
				}
				/** end */
				...
				/** hcai: added to adapt the instrumentation for branch monitors involving exceptional units
				 *  avoid the sanity check for branch target in catch blocks such a branch is not a real "branch" in the original
				 *  program thus target won't be in the unit box (branch target set) of the source
				 */
				/*
				assert found;
				...
				*/
				if (!dua.Options.ignoreCatchBlocks && e.getTgt().isInCatchBlock()) {
					//System.out.println("target in catch block: " + e);
				}
				else	{
					assert found;
				}
