...
public class UtilInstrum {
	...
	public static Stmt getFirstNonIdStmtFrom(PatchingChain pchain, Stmt sFrom) {
	 ...
	 * @param clsReporter reporter class to instantiate
	 * @return local that points to reporter
	 */
	public static Local insertReporterInstantiateCode(SootMethod insertionMethod, SootClass clsReporter, boolean useBooleanCtorArg, boolean cleanUpCovArray) {
		...
		Local reporterLocal = Jimple.v().newLocal("<" + clsReporter.getName() + ">", clsReporter.getType());
		insertBody.getLocals().add(reporterLocal);
		
		// get default ctor method
		SootMethod repCtorMethod = useBooleanCtorArg? clsReporter.getMethod("void <init>(boolean)") : clsReporter.getMethod("void <init>()");
		...
		Stmt reporterInitStmt = Jimple.v().newInvokeStmt(
				useBooleanCtorArg? Jimple.v().newSpecialInvokeExpr(reporterLocal, repCtorMethod.makeRef(), IntConstant.v(cleanUpCovArray? 1 : 0)) :
	...
	/** Initializes reporter at BEGINNING of each entry method. This must be the last instrumentation inserted to ensure initialization before anything else. */
	public static void initializeAtEntryMethods(SootClass clsReporter, List<SootMethod> entryMethods) {
		List probeInit = new ArrayList();
		for (SootMethod entryMethod : entryMethods) {
			...
			Stmt sEntryFirst = UtilInstrum.getFirstNonIdStmt(pchainEntry);
			
			SootMethod mtdInit = clsReporter.getMethod("void initialize()");
			assert mtdInit.isStatic();
			Stmt initDUARepStmt = Jimple.v().newInvokeStmt(
				Jimple.v().newStaticInvokeExpr(mtdInit.makeRef()));
			probeInit.add(initDUARepStmt);
			
			InstrumManager.v().insertAtProbeBottom(pchainEntry, probeInit, sEntryFirst);
		}
	}
