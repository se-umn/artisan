...
public class Monitor {
	...
	public synchronized static void initialize() throws Exception {
		if (dumpCallmap) {
			...
			// cg_writer = new BufferedWriter(new
			// FileWriter(fnCallmap+".sync",hcm.exists()));
		}
		...
		// Avoid double initializations? should we check this at the very top ?
		bInitialized = true;
		//
	...
	/// TODO This shall be extended with parameter values/references !
	...
	public synchronized static void enter_impl(String methodname) {
		try {
			...
			// We need to log the method parameter and such here...
	...
	public synchronized static void returnInto(String methodname, String calleeName, java.lang.reflect.Method refme) {
		...
		active = true;
		try {
			returnInto_impl(methodname, calleeName, refme);
		} finally {
			active = false;
		}
	}
	...
	public synchronized static void returnInto_impl(String methodname, String calleeName,
			java.lang.reflect.Method refme) {
		...
		if (dumpCallmap) {
			...
			if (-1 != idx) {
				...
				if (refme != null) {
					...
					// ceName = utils.getSignature(refme.getDeclaringClass(), refme.getName(),
					// refme.getParameterTypes(), refme.getReturnType());
					ceName = "<" + refme.getDeclaringClass().getName() + ": " + refme.getReturnType().getName() + " "
							+ refme.getName() + "()>";
				}
				callmap.put(methodname, ceName);
				/*
				 * try { cg_writer.write(methodname + " -> " + ceName + "\n"); } catch
	...
	public synchronized static void libCall(String methodname, String calleeName, java.lang.reflect.Method refme) {
		try {
			libCall_impl(methodname, calleeName, refme);
		} finally {
		}
	}
	...
	protected synchronized static void dumpCallmap() {
		if (!fnCallmap.isEmpty()) {
			...
			try {
				...
				// we won't allow the confusion of overwriting the file with the call maps from
				// multiple executions
				fnCallmap = "";
			}
		}
	...
	public static void onRecvSenderID(String senderid) {
		/**
		 * for each process, only record the first message receiving event for each
		 * unique sender
		 */
		...
		for (Integer ts : sortedFL.keySet()) {
			...
			if (F.containsValue(ts)) {
				...
				if (L.containsValue(ts)) {
					// according to the entry event monitor as it is designed, if two method have
					// equal time stamps,
	...
	/**
	 * since the static member will be hidden when inherited by descendants, there
	 * is no point of declaring it as public/protected to let it be inheritable;
	 * Simply speaking, it is associated with no memory object, thus there is no way
	 * to implement polymorphism, which relies on a memory block where the virtual
	 * table can reside for implementing the polymorphism
	 */
	/* protected */ private static void serializeEvents() {
		/*
		 * serialize for later deserialization in the post-processing phase when impact
		 * set is to be computed
		 */
		if (!fnEventMaps.isEmpty()) {
			...
			try {
				...
				// we won't allow the confusion of overwriting the file with the event maps from
				// multiple executions
				fnEventMaps = "";
			}
		}
