/**
 * File: src/utils/TryCatchWrapper.java
 * -------------------------------------------------------------------------------------------
 * Date			Author      Changes
 * -------------------------------------------------------------------------------------------
...
public class TryCatchWrapper {
	 ...
	 *
	 * wrap outermost try{...}catch(Throwable e){throw e} blocks to the given method
	 * so that we can always capture the returnedInto event triggered by the method
	 * even if an uncaught exception occurred in it
	 * 
	 * 1. we add the "Throwable e" catcher, in order to catch all possible
	 * exceptions uncaught; 2. we do nothing in the catch block but to rethrow the
	 * exception, in order to keep the original program semantics unchanged; 3. in
	 * order to make the code still compilable with the "throw e" statement added,
	 * we need make sure the method signature has included "throws Throwable"
	 * declaration, we add this if it is not originally declared as that.
	 * 
	 ...
	 *         java.lang.Throwable { java.lang.String[] r0; java.lang.Throwable r1,
	 *         $r2;
	 * 
	 *         r0 := @parameter0: java.lang.String[];
	 * 
	 *         label0: staticinvoke <ScheduleClass: void
	 *         _main(java.lang.String[])>(r0);
	 * 
	 *         label1: goto label3;
	 * 
	 *         label2: $r2 := @caughtexception; r1 = $r2; throw r1;
	 * 
	 *         label3: return;
	 * 
	...
	/**
	 * by wrapping the outermost "catch (Throwable)" block, ensure that any type of
	 * exceptions and runtime errors can be caught, if thrown, in the given method
	 * 
	 * @param m        the source method where the catch block is added
	 * @param logCatch if insert probe monitoring whether an originally uncaught
	 *                 exception or runtime error really happened in runtime
	...
	public static int wrapTryCatchAllBlocks(SootMethod m, boolean logCatch, boolean debugOut) {
		...
		if (logCatch) {
			...
			sprintArgs.add(StringConstant
					.v("Uncaught throwable caught in " + m.getSignature()/* + "\n Cause is the following:" */));
			Stmt sprint = Jimple.v()
					.newInvokeStmt(Jimple.v().newVirtualInvokeExpr(str1out, mPrintln.makeRef(), sprintArgs));
			tcProbes.add(sprint);

			// print out the cause of the Throwable caught here that would otherwise not be
			...
			if (logThrowableCauses) {
				...
				Stmt causetostring = Jimple.v().newAssignStmt(str1,
						Jimple.v().newVirtualInvokeExpr(tcause, mtostring.makeRef()));
				tcProbes.add(causetostring);
				List<Local> sprintCauseArgs = new ArrayList<Local>();
				sprintCauseArgs.add(str1);
				Stmt sprintCause = Jimple.v()
						.newInvokeStmt(Jimple.v().newVirtualInvokeExpr(str1out, mPrintln.makeRef(), sprintCauseArgs));
			...
			if (logStackTrace) {
				...
				Stmt sprintstack = Jimple.v()
						.newInvokeStmt(Jimple.v().newVirtualInvokeExpr(er1, mPrintStackTrace.makeRef(), str1out));
		...
		if (sLast instanceof IdentityStmt) {
			System.out.println("wrapTryCatchAllBlocks: special case encountered in method " + m.getSignature()
					+ " stmt: " + sLast);
			/*
	 ...
	 *
	 * wrap outermost try{...}catch(Exception e){throw e} blocks to the given method
	 * so that we can always capture the returnedInto event triggered by the method
	 * even if an uncaught exception occurred in it
	 * 
	 * 1. we add the "Exception e" catcher, in order to catch all possible
	 * exceptions uncaught; 2. we do nothing in the catch block but to rethrow the
	 * exception, in order to keep the original program semantics unchanged; 3. in
	 * order to make the code still compilable with the "throw e" statement added,
	 * we need make sure the method signature has included "throws Exception"
	 * declaration, we add this if it is not originally declared as that.
	 * 
	 * @param m the method to be wrapped around the try-catch blocks
	 * @return 0 for success, and failure otherwise
	 ...
	 * 
	 *         public static void main(java.lang.String[]) throws
	 *         java.lang.Exception { java.lang.String[] r0; java.lang.Exception r1,
	 *         $r2;
	 * 
	 *         r0 := @parameter0: java.lang.String[];
	 * 
	...
	public static int wrapTryCatchBlocks(SootMethod m, boolean debugOut) {
		...
		// add "throws Exception" declaration if absent so that we add exception
		// rethrowing statement in the catch block to be added
